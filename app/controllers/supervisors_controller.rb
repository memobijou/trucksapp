class SupervisorsController < ApplicationController
  ...
  before_action :set_supervisor_user, only: [:new, :edit]
  ...
  def new
    ...
    @supervisor.employee_supervisors.build
  end
  ...
  private
    ...
    def supervisor_params
      params.require(:supervisor).permit(:user_id)
    end
    
    def set_supervisor_user
      @supervisor_user = User.where('id NOT IN (?)',Supervisor.select("user_id"))
    end
end
